/*
; CLIP TO SQLITE EXAMPLE
global _AHKVar := "{Int64 mContentsInt64,Double mContentsDouble,PTR mobject},{char *mByteContents,LPTSTR mCharContents},{UINT_PTR mByteLength,_AHKVar *mAliasFor},{UINT_PTR mByteCapacity,UINT_PTR mBIV},BYTE mHowAllocated,mAttrib,mScope,mType,LPTSTR mName"
 
; Backup ClipboardAll data to object
Clipboard:="1"
Clip:=ClipboardAll
Clipboard:="",obj:=[]
obj.SetCapacity(1,sz:=VarSetCapacity(Clip)),RtlMoveMemory(obj.GetAddress(1),&Clip,sz),Clip:=""
 
; Create sqlite table from object
sq:=SQLite3(""),sq.open16[":memory:",hDB],sq.TableFromObj(hDB,[obj],"Test","Clip blob")
 
; Get data from sqlite table to object
obj:=sq.SQLToObj(hDB,"Select * from test",false) ; no column,sq:=""
 
; Restore ClipboardAll
VarSetCapacity(Clip,sz:=obj.1.GetCapacity(1)),  Struct(_AhkVar,getvar(Clip),{mAttrib:1,mByteLength:sz}),  RtlMoveMemory(&Clip,obj.1.GetAddress(1),sz),  Clipboard:=Clip
MsgBox % Clipboard
ExitApp
*/

SQLite3(options:="", dll:=""){
  return new SQLite3(options, dll)
}
Class SQLite3 {
  __Delete(){
  ...
  List(hDB, SQL, BlobToHex:=false){
    ...
    if !(types:=this.SQLType(hDB,SQL)) || !SQL:=this.SQLToObj(hDB,SQL,true)
      return (err:=this.errmsg16[hDB])="not an error"?0:MessageBox(0,err,"Error",0)
    for k,v in SQL.RemoveAt(1)
      columns.=(columns?"|":"") v " (" types[A_Index] ")"
    Loop 10
      If !IsWindowVisible(SQLiteGUI%A_Index%)
        gui:=A_Index
    If !gui
      return MessageBox(0,"Too many Guis, close at least one SQLite Query Winodow","Error",0)
  ...
  Execute(hDB, stmt){
    pzTail:=&stmt,end:=pzTail + StrLen(stmt)*(A_IsUnicode?2:1)
    While pzTail!=end 
    {
  ...
  SQLType(hDB, SQL){
    ...
    pzTail:=&SQL,final:=pzTail + StrLen(SQL)*(A_IsUnicode?2:1),res:=[]
    While pzTail!=final
    {
      if (this.prepare16_v2[hDB,pzTail,-1,pStmt,pzTail])
    ...
    return res,this.finalize[pStmt]
  }
        ...
        While ret=100 
        {
          ...
          Loop % ColumnCount
            ...
            : type = 4 ;|| types[A_Index] = 5 ; use TEXT for NULL
              ? ((!sz:=this.column_bytes[pStmt,A_Index-1]) ? row.Push() : ptr:=this.column_blob[pStmt,A_Index-1],row.SetCapacity(idx:=row.Length()+1,sz),RtlMoveMemory(row.GetAddress(idx),ptr,sz))
  ...
  SQLToString(hDB, SQL, column:=true, sep:="`t", end:="`r`n"){
    pzTail:=&SQL,final:=pzTail + StrLen(SQL)*(A_IsUnicode?2:1)
    While pzTail!=final
    {
      if (this.prepare16_v2[hDB,pzTail,-1,pStmt,pzTail])
        return "",ErrorLevel:=this.errmsg16[hDB]
  ...
  SQLToADO(hDB, SQL, Connection, table, delete:=true, head:=""){
    ...
    If !( conn := ComObjCreate( "ADODB.Connection" ) )
      return MessageBox(0,"Fatal Error: ADODB.Connection is not available.","Error",0)
    ComObjError(true)
    ...
    While pzTail!=final
    {
      ...
      else if ((100=ret:=this.step[pStmt])||ret=101)
      {
        if !ColumnCount{
          ...
          {
            if !delete
              return MessageBox(0,"Table must be deleted to apply header, use true for delete parameter!","Error",0), this.finalize[pStmt],conn.Close(),ComObjError(ComErr)
            If (!conn.Execute(hDB,"Create TABLE " table " (" Trim(head,"()") ")"))
              return MessageBox(0,"Syntax error:`nCreate TABLE " table " (" Trim(head,"()") ")","Error",0),this.finalize[pStmt],conn.Close(),ComObjError(ComErr)
          }
          ...
          if (tbl:=conn.execute(hDB,"SELECT * FROM " table))
          {
            if (ColumnCount != (Fields:=tbl.Fields).Count)
              return MessageBox(0,"Error: Result Query has " ColumnCount " columns but destination table has " oFields.Count " columns.","Error",0),this.finalize[pStmt],conn.Close(),ComObjError(ComErr)
            Loop % ColumnCount
            ...
            If (!conn.Execute(hDB,"Create TABLE " table " (" RTrim(head,",") ")"))
              return MessageBox(0,"Syntax error:`nCreate TABLE " table " (" RTrim(head,",") ")","Error",0),this.finalize[pStmt],conn.Close(),ComObjError(ComErr)
          }
        }
        if ret=101
        {
          if (this.finalize[pStmt])
            return MessageBox(0,"Error Finalize: " this.errmsg16[hDB],"Error",0),conn.Close(),ComObjError(ComErr)
          continue
        }
        If (!record := ComObjCreate( "ADODB.recordset" ))
          return MessageBox(0,"Fatal Error: ADODB.recordset is not available.","Error",0),this.finalize[pStmt],conn.Close(),ComObjError(ComErr)
        record.Open(table,conn,1,3,2) ;adOpenKeyset, adLockOptimistic, adCmdTable
        if (record.State!=1)
          return MessageBox(0,"Error: could not connect to table " table,"Error",0),this.finalize[pStmt],conn.Close(),ComObjError(ComErr)
        ComObjError(true)
        While ret=100 
        {
          record.AddNew()
    ...
    return true, conn.Close(),ComObjError(ComErr)
  }
  ...
  TableFromObj(hDB, source, table, head:="", delete:=true, primary:=""){
    If (delete && this.Execute(hDB,"DROP TABLE IF EXISTS " table "`;CREATE TABLE " table " ( " head (!Primary?"":",PRIMARY KEY (" primary ")") " )"))
      return 0,err:=this.errmsg16[hDB],MessageBox(0,"Error: " err "`n`n" "CREATE TABLE " table " ( " head (!Primary?"":",PRIMARY KEY (" primary ")") " )","Error",0)
    ...
    if (this.prepare16_v2[hDB,&("INSERT INTO " table " VALUES (" RTrim(stmt,",") ")"),-1,pStmt])
      return MessageBox(0,"Error prepare " table "`n" err,"Error",0),err:=this.errmsg16[hDB],this.Exec[hDB,"ROLLBACK TRANSACTION"]
    for k,line in source
      If (line.Length()=ColIdx)
      {
        ...
        If (101!=this.step[pStmt])
          return MessageBox(0,"Error step " table "`n" err,"Error",0),err:=this.errmsg16[hDB],this.Exec[hDB,"ROLLBACK TRANSACTION"],this.finalize[pStmt]
        this.reset[pStmt]
      }
    if (this.Exec[hDB,"END TRANSACTION"])
      return MessageBox(0,"Error end transaction " table,"Error",0),this.finalize[pStmt]
    if (this.finalize[pStmt])
      return MessageBox(0,"Error finalize " table,"Error",0)
    return true
  }
  TableFromFile(hDB, source, table, head:="", Delimiter:="`t", skip:=0, trim:="", delete:=true, primary:=""){
    FileGetSize,sz,%source%
    If (!FileExist(source)||!sz)
      return MessageBox(0,"File " source " does not exist or is empty","Error",0)
    If (delete && this.Execute(hDB,"DROP TABLE IF EXISTS " table "`;CREATE TABLE " table " ( " head (!Primary?"":",PRIMARY KEY (" primary ")") " )"))
      return 0,err:=this.errmsg16[hDB],MessageBox(0,"Error: " err "`n`n" "CREATE TABLE " table " ( " head (!Primary?"":",PRIMARY KEY (" primary ")") " )","Error",0 )
    colType:=this.TableDeclType(hDB,table)
    Loop % ColIdx:=colType.Length()
    ...
    this.Exec[hDB,"BEGIN TRANSACTION"]
    if (this.prepare16_v2[hDB,&("INSERT INTO " table " VALUES (" RTrim(stmt,",") ")"),-1,pStmt])
      return MessageBox(0,"Error prepare " table "`n" err "`nINSERT INTO " table " VALUES (" RTrim(stmt,",") ")" "Error",0),err:=this.errmsg16[hDB],this.Exec[hDB,"ROLLBACK TRANSACTION"]
    Loop,Read,%source%
  ...
  TableFromADO(hDB, Connection, Query, table, head:="", delete:=1, primary:="" ) {
    If !( conn := ComObjCreate( "ADODB.Connection" ) )
      Return MessageBox(0,"Fatal Error: ADODB is not available.","Error",0)
    ComErr:=ComObjError(false)
    ,conn.ConnectionTimeout := 5 ; Seconds to wait for a connection to open
    ,conn.CursorLocation := 3 ; adUseClient
    ,conn.CommandTimeout := 600 ; 10 minute timeout on the actual SQL statement.
    ,conn.Open( Connection ) ; open the connection.
    if (conn.State!=1)
      return MessageBox(0,"Error: Could not open Connection `n" Connection,"Error",0),ComObjError(ComErr)
    ...
    If ( record := conn.execute( Query ) )
    {
      ...
      If (delete && this.Execute(hDB,"DROP TABLE IF EXISTS " table "`;CREATE TABLE " table " ( " RTrim(head,",") (!Primary?"":",PRIMARY KEY (" primary ")") " )"))
        return 0,record.Close(),conn.Close(),ComObjError(ComErr),err:=this.errmsg16[hDB],MessageBox(0,"Error: " err "`n`n" "CREATE TABLE " table " ( " RTrim(head,",") (!Primary?"":",PRIMARY KEY (" primary ")") " )","Error",0)
      colType:=this.TableDeclType(hDB,table)
      ,this.Exec[hDB,"BEGIN TRANSACTION"]
      if (this.prepare16_v2[hDB,&("INSERT INTO " table " VALUES (" RTrim(stmt,",") ")"),-1,pStmt])
        return MessageBox(0,"Error prepare " table "`n" err,"Error",0),record.Close(),conn.Close(),ComObjError(ComErr),err:=this.errmsg16[hDB],conn.Close(),this.Exec[hDB,"ROLLBACK TRANSACTION"]
            ...
            Loop % ColIdx
              ...
              : columnType="blob" ; || columnType="" ; use TEXT FOR NULL
                ? ((binlen:=Fields.Item( A_Index - 1 ).ActualSize) ; Get SafeArray and pointer to data to process in sqlite3
                  ...
                  : this.bind_null[pStmt,A_Index])
              : InStr(columnType,"double")||InStr(columnType,"real")||InStr(columnType,"floa")
                ? this.bind_double[pStmt,A_Index,Fields.Item( A_Index - 1 ).Value]
              ; if InStr(columnType,"char")||InStr(columnType,"clob")||InStr(columnType,"text")
              : this.bind_text16[pStmt,A_Index,Fields.Item( A_Index - 1 ).Value "",-1,-1] ;SQLITE_TRANSIENT
            If (101!=this.step[pStmt])
              return MessageBox(0,"Error step " table "`n" err,"Error",0),record.Close(),conn.Close(),ComObjError(ComErr),err:=this.errmsg16[hDB],this.Exec[hDB,"ROLLBACK TRANSACTION"],this.finalize[pStmt],conn.Close()
      ...
      return MessageBox(0,RTrim( Err,"`n"),"Error",0),conn.Close(),ComObjError(ComErr)
    }
