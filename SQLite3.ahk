...
sq:=SQLite3("","D:\sqlite3_x86.dll"),sq.open16[":memory:",hDB],sq.TableFromObj(hDB,[obj],"Test","Clip blob")
 
; Get data from sqlite table to object
obj:=sq.SQLToObj(hDB,"Select * from test",false) ; no column,sq:=""
 
; Restore ClipboardAll
VarSetCapacity(Clip,sz:=obj.1.GetCapacity(1)),  Struct(_AhkVar,getvar(Clip),{mAttrib:4,mLength:sz}),  RtlMoveMemory(&Clip,obj.1.GetAddress(1),sz),  Clipboard:=Clip
MsgBox % Clipboard
*/
  ...
  __New(Options:="", dll:=""){
    ...
    If !_library:=ResourceLoadLibrary(dll)
      If !_library:=MemoryLoadLibrary(dll)
        MsgBox("Library " dll " not found"),ExitApp()
